...
$(document).ready(function(){
	...
	function initialize(){
		...
		$('#RemittanceCollectorName').keypress(function(e){
				if(e.which==13){
					$('#RemittanceCollectorName, input').trigger('blur');
				};
			});
	...
	$('#RemittanceRemitted').live('blur', function(){
		...
		var msg = '<center><strong>Invalid amount!..</strong></center>';
	...
	$('#RemittanceRemitted').keypress(function(e){
		if(e.which==13){
			$('#RemittanceRemitted').blur();
		};
	});
	...
	$('#RemittanceCollectorName').blur(function(e){
				if($(this).val()!='' && $('#RemittanceCollector').val()!=''){
					...
					//if($(this).val()!=''){
						var msg = '<center><strong>Collector not authorize!..</strong></center>';
						var button = {};
						button.BACK = function() {
							$(this).dialog('destroy');
							$('#RemittanceCollectorName').select();
