$(document).ready(function(){
		...
		$.getJSON(ssUtil.cch_brk('/canteen/daily_menus/findDailyMenu/'+TODAY+''),
				function(data){
				  ...
				  if(data.length>0){
						...
						var msg = '<br/><center>No Menu For Day Set!</center>';
						var button = {};
						button["View Today's Menu"]=function(){
							top.location.href=('/canteen/daily_menus');
							$(this).dialog('destroy');
						}
	...
	$('#itemCheck').bind('getResult', function(e,a){
		...
		try{
			var menu = a.data;
			console.log(a);
			Li.find('div.id input:first').val(menu.MenuItem.id);
			Li.find('div.desc input:first').val(menu.MenuItem.name);
			Li.find('div.unit input:first').val(menu.Unit.alias);
			Li.find('div.qtyC input:first').val(menu.DailyMenu['0'].served);
			Li.find('div.qty input:first').focus();
		}catch(e){
			var msg ='<center><strong>Menu not found!<br /><br /></strong></center>';
			var button = {};
			button.BACK = function(){
				$(this).dialog('destroy');
				$(code).select().focus();
